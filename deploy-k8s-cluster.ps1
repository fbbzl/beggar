...
elseif ($NodeIps.Count -ge 1) {
    ...
    if ($DryRun) {
        Write-Host "  [DRY-RUN] 跳过 SSH、远程安装、token 读取、等待和 kubeconfig 下载" -ForegroundColor DarkGray
        Write-Host "  [DRY-RUN] SSH 主机密钥策略: $StrictHostKeyChecking; known_hosts: $KnownHostsFile" -ForegroundColor DarkGray
    } else {

    if ($NodeIps.Count -lt 3) {
        Write-Host "[WARN] HA 至少需要 3 个节点，当前为 $($NodeIps.Count) 个节点" -ForegroundColor Yellow
    }
    ...
    foreach ($node in $NodeIps) {
        ...
        if ($LASTEXITCODE -ne 0 -or !$hostname) {
            Write-Host "[FATAL] 无法连接 $node，请检查 SSH 配置" -ForegroundColor Red
